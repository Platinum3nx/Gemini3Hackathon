...
3. **CONTROL FLOW:** Use `split` and `next` for if/else. DO NOT use `case`.
...
def clean_response(text: str) -> str: """ Removes Markdown code fences (lean ... ) from the LLM response. """ text = re.sub(r"^[a-zA-Z]*\n", "", text, flags=re.MULTILINE) text = re.sub(r"$", "", text, flags=re.MULTILINE) return text.strip()

def triage_files(file_list: list[str]) -> list[str]: import json content = f"Files: {json.dumps(file_list)}\n\nIdentify top 3 high-risk files." response_text = call_gemini(TRIAGE_PROMPT, content) try: return json.loads(response_text) except json.JSONDecodeError: print(f"Error parsing triage: {response_text}") return []

def call_gemini(prompt_template: str, content: str) -> str: try: api_key = os.getenv("GEMINI_API_KEY") if not api_key: return "-- Error: GEMINI_API_KEY not found."
